...
public interface CitizenAnimationFactory {
    CitizenAnimationFactory.FactoryHolder ANIMATION_DATA_FACTORY = new CitizenAnimationFactory.FactoryHolder();
    ...
    class FactoryHolder {
        ...
        private static final List<Function<AbstractEntityCitizen, CitizenAnimationFactory.FactoryHolder.DataHolder>> factories = new ArrayList<>();

        public void registerFactory(@Nullable ResourceLocation id, int priority, @NotNull CitizenAnimationFactory factory) {
            factories.add(player -> Optional.ofNullable(factory.invoke(player)).map(animation -> new CitizenAnimationFactory.FactoryHolder.DataHolder(id, priority, animation)).orElse(null));
        }
        ...
        public void prepareAnimations(AbstractEntityCitizen player, AnimationStack playerStack, Map<ResourceLocation, IAnimation> animationMap) {
            for (Function<AbstractEntityCitizen, CitizenAnimationFactory.FactoryHolder.DataHolder> factory : factories) {
                CitizenAnimationFactory.FactoryHolder.DataHolder dataHolder = factory.apply(player);
                if (dataHolder != null) {
